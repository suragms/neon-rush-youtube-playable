...
interface SaveData { best: number; coins: number; settings: Settings; achievements: string[] }
  ...
  constructor() {
    try {
      ...
      if (raw && typeof raw === 'object') {
        ...
        if (Array.isArray(saved.achievements)) this.data.achievements = saved.achievements.filter((id): id is string => typeof id === 'string').slice(0, 32);
      }
